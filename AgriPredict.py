...
if st.button("Submit"):
    ...
    sarimax_model = model.fit(method_kwargs={"disp": False})
    ...
    # Align fitted values index with data index for RMSE calculation
    fitted_values = sarimax_model.fittedvalues
    fitted_values = fitted_values.reindex(data.index)
    train_rmse = np.sqrt(((data - fitted_values) ** 2).mean())
